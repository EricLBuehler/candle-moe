...
// Direct fused kernels for tiny batches (skip preprocessing)
...
namespace fp32_config {
    ...
    constexpr int THREADS_N = BLOCK_N / THREAD_N;
}
...
void fused_moe(
    ...
    // Fast path for tiny batches: skip preprocessing entirely
    // Use direct fused kernels that process (token, expert) pairs independently
    if (num_tokens <= thresholds::DIRECT_MAX_TOKENS) {
        if (is_fp16) {
            if (is_qwen3) {
                qwen3_direct_fused_forward(
...
#ifndef NO_BF16_KERNEL
            if (is_qwen3) {
                qwen3_direct_fused_forward_bf16(
                ...
                nomic_direct_fused_forward_bf16(
                    reinterpret_cast<const __nv_bfloat16*>(input),
                    reinterpret_cast<const __nv_bfloat16*>(gate_weights),
                    reinterpret_cast<const __nv_bfloat16*>(up_weights),
                    reinterpret_cast<const float*>(routing_weights),
            ...
            // FP32 falls through to GEMM path
        }
    }

    size_t compute_elem_size = is_fp32 ? sizeof(float) : sizeof(half);
    // Intermediate buffer uses same type as compute (half for FP16, bf16 for BF16, float for FP32)
    size_t intermediate_elem_size = compute_elem_size;
    size_t align = 256;
    ...
    size_t intermediate_size = ((size_t)total * intermediate_dim * intermediate_elem_size + align - 1) & ~(align - 1);
    ...
    int max_tokens_per_expert;
    const int grid_y_safe_limit = 65535 * 32;  // CUDA grid.y max × smallest BLOCK_M
